...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        for i in range(len(junctions)):
            for j in range(i):
                G.add_edge(junctions[i], junctions[j], weight=self.cached_air_distance_calculator.
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        def air_dist_to_closest_lab(junction: Junction) -> float:
            ...
            """
            assert isinstance(self.problem, MDAProblem)
            return min(map(lambda lab: self.cached_air_distance_calculator.
                           get_air_distance_between_junctions(junction, lab.location),
                           self.problem.problem_input.laboratories))  # TODO: replace `...` with the relevant implementation.

        nr_tests = state.get_total_nr_tests_taken_and_stored_on_ambulance()
        total_dist = air_dist_to_closest_lab(state.current_location) * nr_tests
        total_dist += sum(apart.nr_roommates * air_dist_to_closest_lab(apart.location) for apart in self.problem.get_reported_apartments_waiting_to_visit(state))
        return total_dist
