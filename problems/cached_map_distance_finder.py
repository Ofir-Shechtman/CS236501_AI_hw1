...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        """
        index_tuple = (src_junction.index, tgt_junction.index)
        if self._is_in_cache(index_tuple):
            return self._get_from_cache(index_tuple)
        else:
            problem = MapProblem(self.streets_map, *index_tuple)
            res = self.map_problem_solver.solve_problem(problem)
            if res.is_solution_found:
                self._insert_to_cache(index_tuple, res.solution_cost)
                return res.solution_cost
            else:
                self._insert_to_cache(index_tuple, None)
                return None
