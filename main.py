...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    p2, = ax2.plot(weights, total_nr_expanded, '-r',
                   label='#Expanded states')  # TODO: pass the relevant params instead of `...`.
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    for w in np.linspace(low_heuristic_weight, high_heuristic_weight, n):
        ...
        if res.is_solution_found:
            res_list.append((w, res.solution_g_cost, res.nr_expanded_states))
    plot_distance_and_expanded_wrt_weight_figure(problem.name, *zip(*res_list))
...
def toy_map_problem_experiments():
    ...
    #       solve the same `toy_map_problem` with it and print the results (as before).
    astar_h = AStar(AirDistHeuristic)
    res = astar_h.solve_problem(toy_map_problem)
    print(res)
    # Ex.15
    # TODO:
...
def multiple_objectives_mda_problem_experiments():
    ...
    print(f'DELETE ME: {optimal_distance_cost}')
    eps = 0.6
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    astar_eps = AStarEpsilon(MDAMSTAirDistHeuristic, within_focal_priority_function=within_focal_h_sum_priority_function,
...
def mda_problem_anytime_astar_experiments():
    ...
    astar_anytime = AnytimeAStar(heuristic_function_type=MDAMSTAirDistHeuristic,
                                 max_nr_states_to_expand_per_iteration=1000)
    res = astar_anytime.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
